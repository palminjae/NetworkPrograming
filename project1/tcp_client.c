...
int main() {
    ...
    // 서버에 연결
    if (connect(client_socket_file_descriptor, (struct sockaddr*)&server_address_info, sizeof(server_address_info)) < 0) {
        printf("connect 실패\n");
        exit(1);
    }
    printf("서버 연결 완료!\n");
    ...
    // 데이터 전송 (10번 반복)
    int bytes_sent = send(client_socket_file_descriptor, buff, BUFSIZE, 0);
    if (bytes_sent < 0) {
        printf("send 실패\n");
    }  
    else {
